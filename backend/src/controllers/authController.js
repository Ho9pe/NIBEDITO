  ...
  try {
    ...
    if (isEmail) {
      ...
      try {
        ...
        // 502 rather than 500: nothing here is broken, an upstream dependency
        // is. The detail stays in the log; the client gets something readable
        // and no internals.
        throw createError(
          502,
...
const processRegister = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // Say plainly that nothing was kept, so the address is free to retry and
      // the person is not left wondering whether they half-registered.
      return next(
        createError(
          502,
          "We couldn't send your verification email, so the account was not created. Please try again in a few minutes."
        )
      );
    }
    return successResponse(res, {
...
const changePassword = async (req, res, next) => {
  try {
    ...
    if (newPassword.length < PASSWORD_MIN_LENGTH) {
      throw createError(400, PASSWORD_LENGTH_MESSAGE);
    }
    if (!PASSWORD_PATTERN.test(newPassword)) {
      throw createError(400, PASSWORD_MESSAGE);
    }
